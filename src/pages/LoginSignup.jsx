...
export default function LoginSignup() {
  ...
  const [firstName, setFirstName] = useState("");      // Signup: first name
  const [lastName, setLastName] = useState("");        // Signup: last name
  const [email, setEmail] = useState("");              // Email input
  const [password, setPassword] = useState("");        // Password input
  const [error, setError] = useState("");              // Error messages from API or validation
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-start overflow-hidden">
      ...
      />
      <div className="absolute inset-0 bg-[#142D4C] opacity-30" /> {/* Overlay for dark theme */}
